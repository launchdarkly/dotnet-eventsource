...
namespace LaunchDarkly.EventSource.Internal
{
    public class AsyncHelpersTest : BaseTest
    {
        ...
        public async void AllowCancellationReturnsEarlyWhenCancelled()
        {
            ...
            var deliberateException = new Exception("this should NOT become an unobserved exception");
            Func<Task<int>> taskFn = async () =>
            {
                signal1.Set();
            ...
            try
            {
                ...
                // exception. (There might be some other exceptions in receivedExceptions that
                // are unrelated to this test, which could happen as a side effect of another
                // test running a task that doesn't get immediately cleaned up. We can ignore
                // those here.)
                await Task.Delay(TimeSpan.FromMilliseconds(500));
                GC.Collect();
                GC.WaitForPendingFinalizers();

                Assert.DoesNotContain(deliberateException, receivedExceptions);
            }
            finally
            {
                TaskScheduler.UnobservedTaskException -= exceptionHandler;
